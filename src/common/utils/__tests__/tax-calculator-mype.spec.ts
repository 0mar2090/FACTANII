...
describe('IGV MYPE 10.5% (Ley 32357)', () => {
  it('IGV_RESTAURANT_RATE constant is 0.105', () => {
    expect(IGV_RESTAURANT_RATE).toBe(0.105);
  });

  it('calculates 10.5% IGV when tasaIGV is 0.105', () => {
    ...
    expect(result.igv).toBe(10.5); // 100 * 0.105
    expect(result.precioUnitario).toBe(110.5); // 100 * 1.105
    expect(result.totalItem).toBe(110.5);
  });
  ...
  it('tasaIGV does NOT affect IVAP (tipo 17 always uses 4%)', () => {
    ...
    expect(result.igv).toBe(4); // 4% IVAP, not 10.5%
  });
  ...
  it('calculates correct totals with mixed MYPE rates', () => {
    ...
    expect(totals.igv).toBe(10.5); // 100 * 0.105
    expect(totals.opExoneradas).toBe(200);
    expect(totals.totalVenta).toBe(310.5); // 100 + 200 + 10.5
  });
});
